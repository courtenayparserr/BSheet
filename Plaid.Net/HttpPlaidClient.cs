namespace Plaid.Net
{
    ...
    public class HttpPlaidClient : IPlaidClient
    {
        ...
        /// <inheritdoc />
        public async Task<TokenExchangeResult> ExchangeBankTokenWithOutAsync(string publicToken)
        {
            Condition.Requires(publicToken).IsNotNullOrWhiteSpace();

            ExchangeTokenRequestWithout exchangeRequest = new ExchangeTokenRequestWithout(this.clientId, this.clientSecret, publicToken);
            HttpResponseMessage response = await this.httpClient.PostAsJsonAsync("exchange_token", exchangeRequest);
            string responseJson = await response.Content.ReadAsStringAsync();
